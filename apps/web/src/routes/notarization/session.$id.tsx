...
import { useSession, useJoinSession, useSignDocument, useCompleteNotarization, useUploadRecording } from '../../features/notarization/hooks/useNotarization'
...
function SessionPage() {
  ...
  const [step, setStep] = useState<'location' | 'liveness' | 'mfa' | 'joined'>('location')
  const [location, setLocation] = useState<string | null>(null)
  const [locationType, setLocationType] = useState<0 | 1 | 2>(0) // 0: Philippines, 1: EmbassyConsulate, 2: Other
  ...
  useEffect(() => {
    if (hasJoined && session?.type === 1) { // 1: Ren
      void startLocalStream()
    }
    return () => {
      stream?.getTracks().forEach(track => track.stop())
    }
  }, [hasJoined, session?.type])
  ...
  useEffect(() => {
    if (stream && signalRJoined && session?.type === 1) {
      onUserJoined(() => {
  ...
  const startRecording = () => {
    ...
    recorder.onstop = async () => {
      ...
      const mockFileId = `recording-${id}-${Date.now()}.webm`
      console.log(`[Recording] Uploaded session recording: ${mockFileId}`)
      
      // Link recording to session in backend
      await uploadRecording.mutateAsync({ id, fileId: mockFileId })
  ...
  const handleJoin = async () => {
    const mockLocation = locationType === 1 
      ? "Philippine Embassy, Tokyo, Japan" 
      : "14.5995° N, 120.9842° E (Manila, Philippines)"
    
    setLocation(mockLocation)
    await joinSession.mutateAsync({ id, location: mockLocation, locationType })
    setStep('liveness')
  }
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-stone-50 p-6">
        <div className="w-full max-w-md space-y-8 rounded-3xl border border-stone-200 bg-white p-8 text-center shadow-sm">
          {step === 'location' && (
            <>
              <h1 className="text-2xl font-semibold">Step 1: Location Verification</h1>
              <p className="text-stone-600">
                To proceed, we must verify your current physical location as required by the 2026 Rules on Electronic Notarization (A.M. No. 24-10-14-SC).
              </p>

              <div className="space-y-4 text-left">
                <label className="text-xs font-semibold uppercase tracking-wider text-stone-500">Declared Location</label>
                <select 
                  value={locationType}
                  onChange={(e) => setLocationType(Number(e.target.value) as any)}
                  className="w-full rounded-xl border border-stone-200 bg-stone-50 p-3 text-sm"
                >
                  <option value={0}>Inside the Philippines</option>
                  <option value={1}>Philippine Embassy / Consular Office</option>
                  <option value={2}>Other Location (Prohibited for REN)</option>
                </select>
              </div>
              ...
              >
                {joinSession.isPending ? 'Verifying...' : 'Verify Location & Next'}
              </button>
            </>
          )}
          ...
          {step === 'liveness' && (
            <>
              <h1 className="text-2xl font-semibold">Step 2: Liveness Detection</h1>
              <p className="text-stone-600">
                Please look at the camera and blink. This ensures you are a live person (Rule VII, Sec 2, d).
              </p>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                   {isVerifyingLiveness ? (
                     <div className="flex flex-col items-center gap-2">
                        ...
                        <span className="text-xs font-medium uppercase tracking-widest text-stone-900">Scanning Face...</span>
                     </div>
                   ) : (
                     <div className="h-32 w-32 rounded-full border-2 border-dashed border-stone-300" />
                   )}
                </div>
              </div>
              ...
              <button
                ...
                className="w-full rounded-2xl bg-stone-900 py-3 text-sm font-medium text-white transition hover:bg-stone-800 disabled:opacity-50"
              >
                {isVerifyingLiveness ? 'Detecting...' : 'Start Liveness Check'}
              </button>
            </>
          )}
          ...
          {step === 'mfa' && (
            <>
              <h1 className="text-2xl font-semibold">Step 3: Multi-Factor Auth</h1>
              <p className="text-stone-600">
                We've sent a code to your registered device. Enter it below to authorize this session (Rule VII, Sec 2, c).
              </p>
              ...
              <div className="space-y-4 text-left">
                ...
                <input 
                  ...
                  maxLength={6}
                  className="w-full rounded-xl border border-stone-200 bg-stone-50 p-4 text-center text-2xl font-bold tracking-[1em]"
                />
              </div>
    ...
    <div className="flex h-screen flex-col bg-stone-900 text-white">
      ...
      <header className="flex items-center justify-between border-b border-stone-800 px-6 py-4">
        <div className="flex items-center gap-4">
          {session.type === 1 ? (
            <span className="rounded-full bg-red-500 px-2 py-1 text-[10px] font-bold uppercase tracking-widest animate-pulse">Recording</span>
          ) : (
            <span className="rounded-full bg-stone-700 px-2 py-1 text-[10px] font-bold uppercase tracking-widest">In-Person</span>
          )}
          <h1 className="text-sm font-medium">Session: {session.id.slice(0, 8)}...</h1>
        </div>
        <div className="flex items-center gap-4 text-xs">
          <span className="rounded-full bg-green-500/20 px-2 py-1 text-green-500 font-medium">
             {session.type === 1 ? 'Verified Location' : 'In-Person Venue'}: {location || session.principalLocation}
          </span>
          <div className="h-4 w-px bg-stone-800"></div>
          <span className="text-stone-400">{isEnp ? 'Acting as Electronic Notary Public' : 'Acting as Principal'}</span>
        </div>
      </header>
      ...
      <main className="flex flex-1 overflow-hidden">
          ...
          {session.type === 1 ? (
            <div className="grid h-full w-full gap-4 md:grid-cols-2">
              ...
              <div className="relative flex items-center justify-center overflow-hidden rounded-3xl bg-stone-800">
                ...
                <div className="absolute bottom-4 left-4 flex items-center gap-2 rounded-lg bg-black/40 px-2 py-1 text-[10px]">
            ...
            <div className="flex flex-col items-center text-center space-y-6">
               <div className="h-32 w-32 rounded-full bg-stone-800 flex items-center justify-center">
                  <UserIcon />
               </div>
               <div>
                  <h2 className="text-2xl font-semibold">In-Person Session</h2>
                  <p className="text-stone-500 mt-2">Parties are physically present at {session.principalLocation}.<br/>Electronic documents are being reviewed on this shared facility.</p>
               </div>
            </div>
          )}
        ...
        <div className="flex w-96 flex-col border-l border-stone-800 bg-stone-900">
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            <section>
              <h2 className="text-[10px] font-bold uppercase tracking-widest text-stone-500">Documents</h2>
            ...
            {session.witnesses.length > 0 && (
              <section>
                <h2 className="text-[10px] font-bold uppercase tracking-widest text-stone-500">Witnesses</h2>
                <div className="mt-4 space-y-3">
                  {session.witnesses.map((witness) => (
                    <div key={witness.id} className="flex items-center gap-3 rounded-xl bg-stone-800/50 p-3">
                      <div className="h-8 w-8 rounded-full bg-stone-700 flex items-center justify-center text-[10px] font-bold">
                        {witness.fullName.charAt(0)}
                      </div>
                      <div>
                        <p className="text-xs font-medium">{witness.fullName}</p>
                        <p className="text-[10px] text-stone-500">Verified Witness</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="border-t border-stone-800 p-6">
            <p className="text-[10px] text-stone-500 leading-relaxed">
              Rule VII, Sec 2, s: This facility uses tamper-evident security measures including end-to-end encryption and digital signatures.
            </p>
          </div>
        </div>
      </main>
...
function UserIcon() { return <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> }
